...
export default function Dashboard() {
  ...
  const handleDeleteBlog = async (id: number) => {
    // User se confirm karo delete karne se pehle
    if (!window.confirm("Are you sure you want to delete this blog?")) return;

    try {
      // API call with DELETE method aur ID query parameter me
      ...
      if (data.success) {
        // Agar delete ho gaya to success message dikhao aur list update karo
        alert("Blog deleted successfully!");
        fetchBlogs(); // Ye function tumhari list ko wapas refresh kar dega
      } else {
        // Agar backend se error aayi
  ...
  useEffect(() => {
    if (activePage === "blogs" && blogView === "list") {
      fetchBlogs();
    }
  }, [activePage, blogView]);
