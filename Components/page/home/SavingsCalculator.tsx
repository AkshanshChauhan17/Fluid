...
export default function SavingsCalculator() {
  ...
  const [statementFees, setStatementFees] = useState("");
  ...
  const handleReveal = async () => {
    try {
      ...
      form.append("monthly_volume", monthlyVolume);
      form.append("statement_fees", statementFees);
      form.append("current_provider", "Pending");
      form.append("practice_type", "Pending");
      form.append("contact", "Pending");

      const response = await fetch("https://api.fluid.financial/hc.php", {
        method: "POST",
        body: form,
      });
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      form.append("monthly_volume", monthlyVolume);
      form.append("statement_fees", statementFees);
      form.append("current_provider", provider);
      ...
      >
        ...
        <div className="w-full h-[45px] bg-[#F0F7FF] border-b border-[#EEEEEE] px-[18px] sm:px-[32px] flex items-center justify-between">
          ...
          <div className="w-[128px] h-[8px] bg-[#EEEEEE] rounded-full overflow-hidden">
            <motion.div
              animate={{
                width: showFinalStep || showLoading ? "100%" : "50%",
              }}
                  ...
                  <div className="flex flex-col gap-[6px]">
                    ...
                    <div className="flex flex-col gap-[8px]">
                      ...
                      <div className="flex items-start gap-[5px]">
                        <Info size={13} className="text-[#73797B] mt-[3px]" />
                    ...
                    <div className="flex flex-col gap-[8px]">
                      <div className="w-full h-[52px] border border-[#D0D5DD] rounded-[10px] px-[16px] flex items-center gap-[8px] bg-white">
                        ...
                        <input
                          type="text"
                          value={statementFees}
                          onChange={(e) => setStatementFees(e.target.value)}
                          placeholder="0.00"
                          className="flex-1 outline-none bg-transparent text-[16px] text-black placeholder:text-[#73797B]"
                        />
                      </div>
                    ...
                    return (
                      ...
                      >
                        {item.loading ? (
                          ...
                          >
                            <Icon size={18} className="text-[#3B747F]" />
                          </motion.div>
                        ) : (
                          <Icon
                            size={18}
                            className={item.completed ? "text-[#22C55E]" : "text-[#A0A8B5]"}
                          />
                        )}
                ...
                {!lastRes ? (
                  <div>
                    ...
                    <div className="w-full max-w-[506px] flex flex-col gap-[20px] mt-[24px]">
                      {/* PROVIDER */}
                      <motion.div whileFocus={{ scale: 1.01 }} className="flex flex-col gap-[8px]">
                      ...
                      <motion.div whileFocus={{ scale: 1.01 }} className="flex flex-col gap-[8px]">
                        ...
                        <div className="relative">
                          ...
                          <ChevronDown size={18} className="absolute right-[16px] top-1/2 -translate-y-1/2 text-[#73797B] pointer-events-none" />
                        </div>
                      </motion.div>
                      ...
                      >
                        ...
                        />
                        <Upload size={28} className="text-[#73797B]" />
                        <p className="mt-[14px] text-[#1D3855] text-[14px] font-semibold">
                          {fileName ? fileName : "Upload a statement (optional)"}
                        </p>
                      ...
                      <motion.button
                        ...
                        className="w-full h-[54px] bg-[#3B747F] rounded-[10px] flex items-center justify-center transition-all duration-300 disabled:opacity-70"
                      >
                        <span className="text-white text-[16px] font-semibold">
                  ...
                  <SuccessMessage 
                    data={
                      <div className="flex flex-col items-center text-center gap-4 py-6 w-full max-w-[506px]">
                        <CheckCircle2 size={64} className="text-[#3B747F] mb-2" />
                        <h2 className="text-[#1D3855] text-[28px] font-semibold leading-tight">
                          Thank you for submitting
                        </h2>
                        <p className="text-[#73797B] text-[16px] mb-4">
                          Our team will review your information and will return with your custom audit shortly.
                        </p>
                        
                        {/* Optional: You can keep the Estimated Savings Report visible below the message if desired */}
                        {result && (
                          <div className="w-full text-left bg-[#F0F7FF] border border-[#D9EAF7] rounded-[18px] p-[20px] sm:p-[24px] mt-4">
                            <h3 className="text-[#1D3855] text-[20px] font-semibold">
                              Estimated Savings Report
                            </h3>
                            <div className="mt-[18px] flex flex-col gap-[12px]">
                              <div className="flex justify-between">
                                <span className="text-[#73797B]">Effective Rate</span>
                                <strong className="text-[#1D3855]">{result.effective_rate}%</strong>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-[#73797B]">Monthly Savings</span>
                                <strong className="text-[#1D3855]">${result.monthly_savings}</strong>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-[#73797B]">Yearly Savings</span>
                                <strong className="text-[#1D3855]">${result.yearly_savings}</strong>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-[#73797B]">Risk Score</span>
                                <strong className="text-[#1D3855]">{result.risk_score}%</strong>
                              </div>
                            </div>
                          </div>
                        )}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-center w-12 h-12 rounded-full bg-[#F0F7FF]">
                    <Icon size={22} className="text-[#5DA7CF]" strokeWidth={1.5} />
                  </div>
